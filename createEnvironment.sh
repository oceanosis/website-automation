...
create-stack()
{
	aws cloudformation create-stack --stack-name $1 --template-body file://$basefld/$subfld/$2 --region="$region"  2>&1
}
delete-stack()
{
...
sed -i 's/.*VpcId":.*/"VpcId": "'$VPCID'",/g' $basefld/$subfld/subnet.json
...
# Update security
sed -i 's/.*VpcId":.*/"VpcId": "'$VPCID'",/g' $basefld/$subfld/security.json
sed -i 's/.*SubnetId":.*/"SubnetId": "'$SUBNETID'",/g' $basefld/$subfld/security.json
